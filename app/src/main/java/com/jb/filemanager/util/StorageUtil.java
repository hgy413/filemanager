...
public class StorageUtil {
     ...
     * 如果sd卡不可用则返回rom空间大小
     */
    public static SDCardInfo getSDCardInfo(Context ctx) {
        SDCardInfo sdcard = new SDCardInfo();
        Set<String> paths = getAllExternalPaths(ctx);
        if (paths != null && !paths.isEmpty()) {
            for (String path : paths) {
                Logger.d(TAG, "SD卡路径 -- " + path);
    ...
    private static long getPathTotalSize(String path) {
        ...
        if (!TextUtils.isEmpty(path)) {
            ...
            try {
                ...
                Logger.d(TAG, "获取总空间出错：" + Log.getStackTraceString(e));
                size = 0;
            }
            if (sf != null) {
    ...
    public static long getPathFreeSize(String path) {
        ...
        if (!TextUtils.isEmpty(path)) {
            ...
            try {
                ...
                Logger.d(TAG, "获取空闲空间出错：" + Log.getStackTraceString(e));
                size = 0;
            }
            if (sf != null) {
    ...
    public static Set<String> getAllExternalPaths(Context context) {
        ...
        try {
            if (Machine.HAS_SDK_ICS) {
                Object storageObject = context
                        .getSystemService(Context.STORAGE_SERVICE);
                Class<?> smCLass = Class
                        .forName("android.os.storage.StorageManager");
                Class[] arrayOfClass = new Class[0];
                Method localMethod = smCLass.getMethod("getVolumePaths",
                        arrayOfClass);
                Object[] arrayOfObject = new Object[0];
                String[] returnFilePath = (String[]) localMethod.invoke(
                        storageObject, arrayOfObject);
            ...
            InputStreamReader fiReader = new InputStreamReader(fileIn);
