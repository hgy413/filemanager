...
import com.jb.filemanager.statistics.bean.Statistics41Bean;
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    // 用于格式化日期,作为日志文件名的一部分
    private DateFormat mFormatter = new SimpleDateFormat("yy-MM-dd_HH-mm-ss", Locale.getDefault());
    ...
    private boolean handleException(Throwable ex) {
        ...
        // 收集设备参数信息
        collectDeviceInfo(TheApplication.getInstance());
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        if (context != null) {
            // 上传41 Crash协议
            Statistics41Bean bean = new Statistics41Bean(context);
            if (result.indexOf("\n") > 0) {
                bean.mOperateCode = result.substring(0, result.indexOf("\n"));
            }
            bean.mRemark1 = logs;
            StatisticsManager.getInstance(context).upLoadStaticData(bean.toFormatStatisticsData());
        }
    ...
    private void printLog(String fileName) {
        if (!new File(fileName).exists()) {
            Toast.makeText(TheApplication.getInstance(), "日志文件不存在！", Toast.LENGTH_SHORT).show();
            return;
        }
